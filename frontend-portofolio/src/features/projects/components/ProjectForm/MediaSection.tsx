...
export default function MediaSection({
    ...
    const handleCropComplete = async (croppedAreaPixels: Area) => {
        ...
        try {
            ...
            const croppedFile = await getCroppedImg(cropImage, croppedAreaPixels, 'cropped.jpg')
        ...
        <section className="space-y-6">
            ...
            <div>
                ...
                <div className="flex gap-4">
                    <input name="cover_url" value={formData.cover_url} onChange={onInputChange} className={`${inputClass} flex-1`} placeholder="https://..." />
                    <input type="file" accept="image/*" hidden ref={coverInputRef} onChange={handleCoverSelect} />
                    <button
                        type="button"
                        onClick={() => coverInputRef.current?.click()}
                        disabled={uploading}
                        className="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-lg font-medium transition-colors disabled:opacity-50 flex items-center gap-2 whitespace-nowrap"
                    >
            ...
            {/* Gallery Images */}
            <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Gallery Images</label>
                <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
                    ...
                    {uploading && targetType === 'gallery' && (
                        <div className="mb-4 text-center text-sm text-primary-600 animate-pulse font-medium">Processing cropped image...</div>
                    )}
            ...
            {/* Content Body */}
            <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Content Body (Markdown) <span className="text-red-500">*</span></label>
                <textarea name="body" value={formData.body} onChange={onInputChange} required rows={15} className={`${inputClass} font-mono text-sm`} />
            </div>
